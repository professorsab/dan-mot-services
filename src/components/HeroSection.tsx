...
export default function HeroSection() {
    ...
    <section className=" min-h-screen  m-0 flex items-center justify-center overflow-hidden">
      ...
      <div className="absolute inset-0  bg-black/60">
          {[...Array(6)].map((_, i) => (
            <div
              key={i}
              className="absolute w-4 h-4 bg-red-500/30 rounded-full animate-float"
              style={{
                left: `${20 + i * 15}%`,
                top: `${30 + i * 10}%`,
      ...
      {/* Content */}
      <div className="relative z-10 container mx-auto px-4 bottom-[-70px] text-left text-white">
        <h1 className="text-5xl md:text-4xl lg:text-6xl font-bold mb-6 leading-tight">
          Expert Repairs,
          <br />
          <span className="text-red-500 typewriter inline-block max-w-fit">Trusted Care</span>
        </h1>
 ...
 <div className={`transition-all duration-1000 delay-400 ${isVisible ? "animate-bounce-in" : "opacity-0"}`}>
            <Button
              size="lg"
              className="bg-red-500 hover:bg-red-600 text-white px-10 py-4 text-lg font-semibold rounded-lg transition-all duration-300 transform hover:scale-110 hover:shadow-2xl animate-pulse-glow relative overflow-hidden group"
            >
